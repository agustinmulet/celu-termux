...
echo ""
select app in utils lidarr prowlarr radarr readarr whisparr quit; do
...
### UTILS INSTALLATION
if [[ $app == 'utils' ]]; then
    echo -e ${yellow}"   Installing utils"${reset}
    echo ""
...
echo ""
echo "Removing old service file from both init.d and systemd..."
rm -rf /etc/systemd/system/"$app".service
rm -rf /etc/init.d/"$app"
sleep 2

# Create app .service with correct user startup
echo ""
echo "Creating new service file..."
cat <<EOF | tee /etc/init.d/"$app" >/dev/null
#!/bin/bash
...
### END INIT INFO
    ...
    do_start() {
            ...
            if pgrep -f "^\${DAEMON}" > /dev/null 2>&1; then
                    ...
                    start-stop-daemon -q -c \$RUNAS --start --background --make-pidfile --pidfile \$PIDFILE --exec .\$DAEMON \$DAEMON_OPTS
                    RETVAL=\$?
            fi
            log_end_msg \$RETVAL
    }
...
while ! service "$app" status > /dev/null; do
    sleep 1
done
